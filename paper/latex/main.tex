...
Generative AI was used substantially in this work, and the following describes
the extent.

An AI coding assistant (Claude, Anthropic) was used under the author's direction
for: implementing the de-obfuscation scan and the \texttt{source} trust-boundary
mechanism; writing the harnesses that reproduce Open-Prompt-Injection, BIPIA and
NotInject; running the experiments; and producing the first draft of this text,
including its tables. Several findings reported here surfaced during that work,
among them the INT8 quantization collapse in Section~ef{sec:quant} and the
evaluation contamination in Section~ef{sec:heldout}.

The author set the research direction, decided what to measure, made the design
decisions the paper reports (notably shipping Tier~2 monitor-only rather than
blocking), and is responsible for all claims. Every quantitative claim is
generated from committed result artifacts by a checking script rather than
transcribed, and the reference list was assembled from retrieved papers rather
than from model recall, both because model-produced numbers and citations are
unreliable. Remaining errors are the author's.
